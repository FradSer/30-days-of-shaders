...
// http://glslsandbox.com/e#56377.0
...
void main( ) {
    ...
    float radius = mix( .1, .4,
        sin( length( gv - id ) - u_time * 3. ) * .5 + .5 ); // how to offset sine based on id

    float m = smoothstep( radius, radius * .5, d );

    pattern += m;

    vec3 color = vec3( snoise( id ));
